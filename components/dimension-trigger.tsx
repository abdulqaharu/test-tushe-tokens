...
export const DimensionTrigger = forwardRef<
  HTMLButtonElement,
  { label: string; value: string; swatch: React.ReactNode } & React.ButtonHTMLAttributes<HTMLButtonElement>
...
export function SolidSwatch({ colorClassName }: { colorClassName: string }) {
  return <span className={`size-4.5 rounded-full border border-white ${colorClassName}`} />;
}
...
export function GradientSwatch({ fromHex, toHex }: { fromHex: string; toHex: string }) {
  return (
    <span
